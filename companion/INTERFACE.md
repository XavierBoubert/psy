...
4. 🔴 **Elle n'apparaît qu'après une publication** — une chose que Xavier a lui-même décidée, et qui lui a été annoncée dans la conversation juste avant. **Ce n'est pas Kokoro qui vient vers lui : c'est le programme qui a changé parce qu'ils l'ont changé ensemble.** *(Une publication de documentation peut survenir hors séance ; c'est l'annonce, pas le rythme hebdomadaire, qui tient cette condition.)*
...
| **Barre de statut et volet de notifications** | `drawable-*/ic_kokoro.png` — ⭐ **le visage seul**, pas le logo | Remplace **le cercle vide** qui tenait la place depuis K2 |
...
| ⭐ **Les deux icônes en aplat ne sont pas un détourage** | Une icône de notification est **repeinte d'une seule couleur** par le système : une photo y devient une tache. **On garde les aplats clairs et on perce les traits** — yeux, sourire, contours redeviennent des trous. Pour la couche monochrome le tri se fait sur la « froideur » du pixel *(bleu − rouge)* : le fond cyan la pousse au-delà de 45, la crème du personnage la rend négative, le gris des traits reste sous 20 |
| ⭐ **L'icône de notification ne vient pas du logo, mais du visage dessiné à part** | `companion/ressources/retenus/kokoro-face.svg`. **À 24 dp le personnage entier n'est plus lisible** : la main, le corps et le 心 s'écrasent en une tache. Le visage seul garde ses yeux et son sourire. Le SVG est rastérisé en grand, percé de la même façon, **rogné sur ses pixels opaques** puis centré — il occupe donc toute la toile utile, sans dépendre de la marge de son cadre de dessin |

**Tout est fabriqué par [`companion/scripts/companion-icone.ts`](scripts/companion-icone.ts)** — `npm run companion:icone` — **en cinq densités**, à partir de ces deux sources. 🔴 **Aucune image d'icône ne se retouche à la main** : le dessin change, on relance le script.
