...
> 📌 **Écart repéré avec `SCHEMA.md` §6, à trancher avec Xavier avant correction.** L'exemple du schéma porte `"version": "VVIQ-2"` avec un `score_max` de 80. Or le **VVIQ-2** (Marks, 1995) compte **32 items** (chaque scène évaluée les yeux ouverts puis fermés) et plafonne à **160**. L'exemple est donc incohérent avec lui-même. La version retenue ici est le **VVIQ 16 items /80**, qui correspond au `score_max` du schéma. Le schéma étant normatif, sa correction doit être **annoncée avant d'être appliquée** (`SCHEMA.md` §9) — c'est pourquoi elle est signalée ici et pas faite en silence.
