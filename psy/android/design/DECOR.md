...
Les **planches magenta d'origine** sont versionnées dans [`retenus/decor/`](retenus/decor/) : elles permettent de **re-détourer** sans rien régénérer, donc de revenir sur un seuil sans rejouer le modèle. Les prompts qui les ont produites sont dans [`prompts/`](prompts/) — `_decor.md` (la charte) et `decor-*.md` (une couche par fichier).
...
npm run decoupe -- decor-feuillage/01-b.png \
  psy/android/app/src/main/res/drawable-nodpi/decor_feuillage.webp \
