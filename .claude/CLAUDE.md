...
> 📐 **Toute la doctrine du dispositif tient dans un seul document : [`PLAN.md`](../PLAN.md) (v2.1, 14/08/2026).** Il a absorbé les cinq documents qui se la partageaient — `SYNCHRO.md`, `agent/README.md`, `PLAN-KOKORO.md`, `programme/FORMAT.md`, `dossier/SCHEMA.md`, tous supprimés. **Il n'y a plus qu'un endroit où lire ce que le dispositif est, et un seul où le modifier.**
>
...
| Répertoire | Objet |
|-----------|---------|
| **`PLAN.md`** (racine) ⭐ | 🔴 **Document unique du projet** (v2.1, 14/08/2026), **document courant et non journal de conception**. §1 les cinq personas · §2 les contraintes de Xavier · §3 Claude Psy · **§4 le Superviseur (normatif)** · §5 Kokoro · **§6 le contenu et Google Drive (normatif)** · **§7 le format du dossier (normatif)** · **§8 le format du programme (normatif)** · §9 feuille de route · §10 arbitrages ouverts · §11 journal des décisions |
...
| `kokoro` | ⭐ **Compile, teste, installe et ouvre Kokoro sur le téléphone** *(14/08/2026)*. Usage : `npm run kokoro` · `npm run kokoro -- pose` (sans repasser les tests) · `-- test` · `-- apk` · `-- journal` · `-- plantage`. **C'est le même script que `./kokoro`**, appelable depuis la racine — il existe pour que **Xavier déploie sans passer par Claude**. Détail : `psy/android/README.md` |
| `image` | Génère des planches de recherche graphique via Gemini. Usage : `npm run image -- <variante> [--base=<charte>] [--format=…] [--taille=1K\|2K\|4K] [--n=…]`. Prompts dans `psy/android/design/prompts/` — **`_base.md` pour le personnage, `_decor.md` pour le décor**. Sorties dans `design/sorties/` *(non versionné)* ; **seul `design/retenus/` fait foi** |
| `decoupe` | ⭐ **Détoure une planche générée** : le fond magenta `#FF00FF` posé par le modèle devient le canal alpha. Usage : `npm run decoupe -- <source> <destination.webp> [--seuil=…] [--plein=…] [--largeur=…]`. **Le modèle ne rend pas d'alpha** — motif et algorithme : `psy/android/design/DECOR.md` §3 |
