...
## Pourquoi chacune de ces questions

| # | Question | Fondement |
|---|---|---|
| 1 | **Alprazolam + SAOS** | Les benzodiazépines dépriment la commande respiratoire centrale, émoussent la réponse d'éveil à l'hypoxie et diminuent le tonus des muscles dilatateurs des voies aériennes supérieures — les trois mécanismes qui aggravent précisément un SAOS obstructif. Une étude retrouve un risque majoré d'insuffisance respiratoire aiguë chez les patients SAOS exposés aux benzodiazépines. C'est la question la plus concrète de l'email : elle porte sur un médicament pris **aujourd'hui**, dans un contexte de SAOS sévère **documenté**. |
| 2 | **Venlafaxine + mouvements périodiques des jambes** | La venlafaxine figure parmi les antidépresseurs les plus associés à l'apparition ou à l'aggravation d'un syndrome des jambes sans repos et à l'augmentation des mouvements périodiques. **Nuance à garder en tête : la revue systématique conclut que ces effets restent globalement peu fréquents** — d'où une formulation en question, sans alarmisme. Le dosage de la **ferritine** est le bilan de première intention du SJSR (cible usuelle > 75 µg/L) et n'a apparemment jamais été fait. Xavier a 31 MPJ/h et des impatiences notées au contexte clinique : le signal existe. |
...
Les deux diagnostics ne sont pas indépendants — ils s'alimentent :

```
SAOS sévère (IAH 35, 61 micro-éveils/h)
   │
   ├──> hypoxie intermittente nocturne ────────────> aggrave la NASH
   │
   ├──> privation de sommeil profond ──────────────> dérègle ghréline / leptine
   │                                                  = appétit ↑, satiété ↓
   │                                                        │
   │                                                        v
   │                                          s'ajoute au déficit intéroceptif
   │                                          (satiété déjà non perçue)
   │                                                        │
   │                                                        v
   └────────────────── prise de poids <────────────────────┘
                            │
                            ├──> aggrave le SAOS (boucle fermée)
                            └──> aggrave la NASH
```

**Conséquence stratégique : traiter correctement le SAOS n'est pas seulement un enjeu de fatigue — c'est un levier sur le poids, donc sur le foie.** La privation de sommeil dérègle les hormones de l'appétit chez tout le monde ; chez quelqu'un qui ne perçoit déjà pas sa satiété, les deux mécanismes convergent sur la même fonction. C'est probablement le point d'entrée le plus rentable de tout le programme d'hygiène de vie.

**Et symétriquement : perdre du poids améliore le SAOS.** Les deux prescriptions — celle du Dr Roisman (« perte de poids souhaitable ») et celle de la Dr Bouarioua (« perdre du poids +++++ ») — ont été formulées indépendamment par deux spécialistes qui ne se sont pas concertés. Elles convergent.
